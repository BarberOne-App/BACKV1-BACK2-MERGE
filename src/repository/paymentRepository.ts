...
export async function getPaymentSummary(barbershopId: string) {
  ...
  const paid = all.filter((r: { status: string; }) => r.status === 'paid' || r.status === 'approved')
    .reduce((sum: any, r: { _sum: { amount: any; }; }) => sum + toNum(r._sum.amount), 0);
  const pending = all.filter((r: { status: string; }) => r.status === 'pending')
    .reduce((sum: any, r: { _sum: { amount: any; }; }) => sum + toNum(r._sum.amount), 0);
  const refunded = all.filter((r: { status: string; }) => r.status === 'refunded')
    .reduce((sum: any, r: { _sum: { amount: any; }; }) => sum + toNum(r._sum.amount), 0);
