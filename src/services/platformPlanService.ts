...
//     active: plan.active,
//     features: (plan.platform_plan_features ?? []).map((f: any) => f.feature),
...
function serialize(plan: any) {
  return {
    ...
    // Aqui estava o primeiro problema:
    // o include do Prisma vem como platform_plan_features
    features: (plan.platform_plan_features ?? []).map((f: any) => f.feature),
...
async function createPagarmePlanForPlatform(params: {
  ...
  try {
    ...
    if (msg.includes('PAGARME_SECRET_KEY')) {
      throw new AppError(
        503,
        'Integração Pagar.me não configurada no servidor. Defina PAGARME_SECRET_KEY no .env do backend.'
      );
    }
...
async function updatePagarmePlanForPlatform(
...
async function deletePagarmePlanForPlatform(pagarmePlanId: string) {
  try {
    ...
    // Qualquer erro do Pagar.me no DELETE não bloqueia a exclusão local
    console.warn(
      `[Pagar.me] Erro ao excluir plano ${pagarmePlanId} (status ${err?.status}): ${err?.message}`
    );
    return null;
  }
}
...
export async function createPlatformPlanService(params: {
  ...
  // Aqui está o ajuste da criação:
  // A API recebe "features", mas o repository espera "platform_plan_features"
  const dbData = {
    ...data,
    platform_plan_features: features ?? [],
  };

  const shouldSyncPagarme = syncPagarme && isPagarmeConfigured();

  if (!shouldSyncPagarme) {
    const created = await createPlatformPlanInDB(dbData);

    if (!created) {
      throw new Error('Falha ao criar plano da plataforma.');
    }

    return {
      ...serialize(created),
      pagarmeSkipped: !isPagarmeConfigured(),
    };
  }
...
export async function updatePlatformPlanService(
  ...
  if (existing.pagarme_plan_id && isPagarmeConfigured()) {
    const currentPrice = decimalToNumber(existing.price);
    const priceChanged =
      data.price !== undefined && Math.abs(currentPrice - data.price) > 0.001;
  ...
  // Aqui está o ajuste da edição:
  // Se features vier no body, atualiza a tabela relacionada.
  // Se features não vier, mantém as features atuais.
  const updated = await updatePlatformPlanInDB(id, {
    ...dbData,
    ...(features !== undefined
      ? { platform_plan_features: features }
      : {}),
    ...(newPagarmePlanId !== undefined
      ? { pagarmePlanId: newPagarmePlanId }
      : {}),
  });
...
export async function deletePlatformPlanService(id: string) {
  ...
  return {
    message: 'Plano da plataforma removido com sucesso',
  };
}